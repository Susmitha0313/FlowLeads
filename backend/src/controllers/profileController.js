...
// Scraper still uses a shared singleton session (no per-user browser context needed)
...
export const refreshProfile = async (req, res) => {
  try {
    ...
    if (err.message === 'SESSION_EXPIRED') return res.status(401).json({ error: 'SESSION_EXPIRED' });
    res.status(500).json({ error: err.message });
  }
};
